...
epi_data<-read_excel('data/MRU-CHL-CHIK-PCR-Metadata_District-20.06.2025.xlsx')
...
R_estimate<-read_excel('data/Mauritius_CHIKV_R.xlsx')
...
epi_curve<-ggplot()+
  ...
  geom_line(data=R_estimate,aes(date,`Mean(R)`*10,colour='Re'),size=1)+
  geom_ribbon(data=R_estimate, aes(x=date,ymin=`Quantile.0.05(R)`*10,ymax=`Quantile.0.95(R)`*10),fill='purple4',alpha=0.2)+
...
colnames(Mauritius_cases_count_confirmed)<-c("District","case_count")


Mauritius_sequence_count_district<-metadata %>% group_by(District) %>%
  summarise(count=n())
Mauritius_sequence_count_district$District<-str_to_sentence(Mauritius_sequence_count_district$District)
colnames(Mauritius_sequence_count_district)<-c("District","sequence_count")
...
Mauritius_epi_map<-ggplot() + 
  ...
  scale_size_continuous(range = c(4, 10),breaks=c(1,3),labels=c(1,10),name='Genomic\nSampling')+  # Adjust min/max point sizes
...
Mauritius_population<-raster("data/Mauritius_Population_2020.asc")
...
Mauritius_temperature<-raster("data/2023_max_temp_agg_terraclimate_tmax_1958_CurrentYear_GLOBE.nc")
...
precipitation<-raster("data/2023_monthly_precipitation_agg_terraclimate_ppt_1958_CurrentYear_GLOBE.nc")
...
ML_tree <- groupClade(ML_tree,.node=c(1731,1740,1991))

p <- ggtree(ML_tree,aes(color=group),size=0.5) + theme_tree2()+
...
p
#ggsave('ECSA_tree_labels.pdf', width = 50, height = 120, units = "cm",limitsize = FALSE)


NewClade_ML_tree<-read.newick('data/ECSA_subtree_newclade.nwk')

NewClade_ML_tree <- groupClade(NewClade_ML_tree,.node=c(289))
...
residuals<-read.table('data/CentralAfrica_IO_Clade_Tempest.txt',header=T)
...
root_tip<-ggplot(residuals, aes(days, as.numeric(distance)))+
  ...
  annotate(geom = 'text', label='Correlation coefficient = 0.9',x=as.Date("2015-01-30"),y=0.022)+
  annotate(geom = 'text',label='R squared = 0.8', x=as.Date("2015-01-30"),y=0.021)+
...
pop_den<-read.table('data/IO_totaln223_mrun37discreate.skygrid.txt',header=T)
...
library(treeio)
##Figure 4B
beast_tree<-read.beast("data/IO_totaln223_mrun37discreate.skygrid.Combined_MCC.tree")

# Get ggtree data with node info
tree_df <- ggtree(beast_tree, mrsd = "2025-04-20", as.Date = TRUE)$data

# Add tip labels to the tips (isTip == TRUE)
tree_df$label[tree_df$isTip] <- beast_tree@phylo$tip.label


p_tree <- ggtree(beast_tree, mrsd="2025-04-20",as.Date=T, color='navyblue',size=0.7) + theme_tree2()+
  ...
  geom_tippoint(
    ...
    aes(fill = "Mauritius - Pamplemousses"),
    size = 3, stroke = 0.1, color = "black", shape = 21
  ) +
  geom_tippoint(
    data = subset(tree_df, grepl("MAPOU", label, fixed = TRUE)),
    aes(fill = "Mauritius - Riviere du Rempart"),
    size = 3, stroke = 0.1, color = "black", shape = 21
  ) +
  geom_tippoint(
    data = subset(tree_df, grepl("PLAINE", label, fixed = TRUE)),
    aes(fill = "Mauritius - Plaine Wilhems"),
    size = 3, stroke = 0.1, color = "black", shape = 21
  ) +
  geom_tippoint(
  ...
  geom_tippoint(
    ...
    aes(fill = "Mauritius - Black River"),
    size = 3, stroke = 0.1, color = "black", shape = 21
  ) +
  geom_tippoint(
...
age_density<-read.table('data/IO_Clade_age_density.txt',header=T)
...
all_jumps<-read.csv('data/IO_totaln223_mrun37discreate.skygrid.location.history.summary.csv') #done after combining results
#above file obtained by running following command
#java -cp beast-mcmc/build/dist/beast.jar dr.app.tools.TreeMarkovJumpHistoryAnalyzer -mrsd 2025.2 IO_totaln223_mrun37discreate.skygrid.location.history.Combined.trees IO_totaln223_mrun37discreate.skygrid.location.history.summary.csv
...
e<-ggplot(subset(subset(all_jumps_summarized_destinations_mru,endLocation!='Reunion'),endLocation!='Mayotte'),aes(x=month))+
  theme_minimal()+
  scale_fill_manual(values=c("palevioletred3", 'indianred2',"maroon3", "thistle3","mistyrose1","hotpink4","ivory3"),
                    name='Destination')+
...
f<-ggplot(all_jumps_summarized_origins_mru,aes(x=month))+
  theme_minimal()+
  scale_fill_manual(values=c("palevioletred3", 'indianred2',"maroon3", "thistle3","mistyrose1",'royalblue',"hotpink4","ivory3"),
                    name='Origin')+
...
D<-mig_chord(x = pb, 
          ...
          # colours
          grid.col = c("mistyrose1", "royalblue","palevioletred3","indianred2", 'maroon3', "thistle3","hotpink4","ivory3")) 
